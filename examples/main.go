...
func instantPromql() {
	fmt.Printf("---- instant promql ----\n")

	instant := greptime.NewInstantPromql(table)
	promql := &greptime.PromqlRequest{}
	promql.WithInstantPromql(instant)
	body, err := client.PromqlQuery(context.Background(), promql)
	if err != nil {
		fmt.Printf("failed to do promql query: %+v\n", err)
		return
	}
	fmt.Printf("%s\n", string(body))
}
